...
const SetupPassword = () => {
  ...
  if (!token) {
    return (
      <div className="min-h-screen flex flex-col justify-between" dir={isRTL ? "rtl" : "ltr"}>
        <Header />
        <div className="flex-grow flex items-center justify-center">{t('clientAuth.setupPassword.invalidLink')}</div>
        <Footer />
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setError(t('clientAuth.setupPassword.errorMatch'));
      return;
    }
    if (password.length < 6) {
      setError(t('clientAuth.setupPassword.errorLength'));
      return;
    }
    ...
    try {
      ...
      const e = err as any;
      setError(e.response?.data?.message || t('clientAuth.setupPassword.errorDefault'));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow border border-gray-200">
          <div>
            <h2 className="mt-2 text-center text-3xl font-bold text-gray-900">
              {t('clientAuth.setupPassword.title')}
            </h2>
            <p className="mt-4 text-center text-sm text-gray-600">
              {t('clientAuth.setupPassword.subtitle')}
            </p>
          </div>
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="rounded-md shadow-sm space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('clientAuth.setupPassword.passwordLabel')}</label>
                <input
                  type="password"
                  required
                  ...
                  placeholder={t('clientAuth.setupPassword.passwordPlaceholder')}
                  value={password}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('clientAuth.setupPassword.confirmLabel')}</label>
                <input
                  type="password"
                  required
                  className="appearance-none rounded relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-[#1E5ED8] focus:border-[#1E5ED8] sm:text-sm"
                  placeholder={t('clientAuth.setupPassword.confirmPlaceholder')}
            ...
            <div>
              ...
              >
                {loading ? t('clientAuth.setupPassword.loadingBtn') : t('clientAuth.setupPassword.submitBtn')}
              </button>
            </div>
          </form>
        </div>
